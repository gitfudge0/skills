...
Use the project's existing tokens and component patterns where available. Define only missing values or patterns needed for the selected screens. Include states or responsive layouts that the user requested or that are needed to make the selected flow understandable. Content and visual stand-ins should represent the product credibly; mark invented copy, imagery, or data when they could be mistaken for approved material. Use approved assets when available instead of imposing a blanket ban on external imagery.
