...
Read `assets/example.html` for the exact structure, tone, and information density to match, and read `assets/styles.css` for the design system. Build the final report as a single self-contained HTML file:

- Inline the CSS from `assets/styles.css` into a `<style>` tag (don't link it externally — the file needs to work standalone if emailed, moved, or opened offline).
- Follow the section order and component patterns in the example: hero header with the feature name and one-line summary, a short impact paragraph, a strategy stat row explaining the type mix and why, then test cases grouped by type using the `<details>/<summary>` card pattern, then the out-of-scope section.
- Write real content, not placeholder-style text — the impact paragraph and strategy rationale should read like a person who actually looked at this change wrote them, not like generic boilerplate ("This feature is important and should be tested thoroughly").
- Keep it visually restrained per the design system — the point is fast scanning (title + priority + type visible at a glance, detail on click), not a dense wall of prose.

Save the file (name it something like `test-plan-<feature-slug>.html`) and deliver it to the user. Don't paste the full HTML into the chat — present the file.
