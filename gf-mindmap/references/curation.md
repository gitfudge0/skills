...
**The single most important rule of gf-mindmap: no idea from the source may be left
...
- **Lossy (the one that matters here):** you pick a few themes and let the rest
  ...
  the failure gf-mindmap exists to prevent.
...
gf-mindmap map must meet: run the check, drive unaccounted to zero, and be able to
