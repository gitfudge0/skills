...
Stage 8 builds one JSON object and substitutes it for the literal string `__GAPPIE_DATA__` in `assets/report-template.html`, writing the result to `.gappie/report.html`.
...
data = json.loads(pathlib.Path(".gappie/payload.json").read_text())
out = tpl.replace("__GAPPIE_DATA__", json.dumps(data, ensure_ascii=False))
pathlib.Path(".gappie/report.html").write_text(out)
PY
```
