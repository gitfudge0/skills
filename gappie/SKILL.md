...
All artifacts live in `.gappie/` at the project root. Create it if absent.
...
Populate `assets/report-template.html` and write `.gappie/report.html`. The template is self-contained — no network dependencies, works offline, prints sensibly.

Substitute the JSON payload into the `__GAPPIE_DATA__` placeholder. Do not hand-write HTML; the template exists so output is consistent across runs and diffable between them. See `references/report.md` for the payload schema.
