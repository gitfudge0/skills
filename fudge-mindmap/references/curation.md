...
**The single most important rule of fudge:mindmap: no idea from the source may be left
...
- **Lossy (the one that matters here):** you pick a few themes and let the rest
  ...
  the failure fudge:mindmap exists to prevent.
...
fudge:mindmap map must meet: run the check, drive unaccounted to zero, and be able to
