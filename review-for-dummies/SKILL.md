...
Use the **indigo editorial design language** (reference implementation: `/Users/digvijaymahapatra/globus/.gapworkshop/questions.html`; a review-shaped example: `careconvoy-ai-core/.reports/ava-191-review.html`). Its grammar:

- Self-contained page, no external CSS. Indigo body background (`#2D2DD6`) framing a near-white panel (`--panel`) with 2px radius; content column max-width 960px.
- CSS custom properties for every color, with three blocks: `:root` (light), `@media (prefers-color-scheme: dark)` on `:root:not([data-theme])`, and explicit `html[data-theme="dark"]`/`html[data-theme="light"]` overrides. Fixed top-right pill theme-toggle persisting to `localStorage`. `@media print` block forcing light palette, white background, `break-inside: avoid` on items.
- Header, centered: uppercase letter-spaced eyebrow (11px, .24em) → indigo headline (clamp 26–42px, weight 600) that IS the verdict sentence → small meta line (branch · commit · PR link · date).
- Stats row: centered flex of stat items — a 34px outlined SVG circle icon (solid = blockers, dashed = should-fix, dotted = nits, check = clean), a 30px indigo number, a 10px uppercase label. This is layer 1's counts.
- Body: `.erow` sections divided by 1px hairlines — left label column (27%, bold indigo section title + small muted description) and right content column (73%). Sections = finding groups (e.g. "Fixed during review" / "Open — needs a decision" / "Nits" / "Coverage").
- Each finding is an `.item`: 26px outlined numbered circle, then a bold one-line title (layer 2), then layer-3 fields as micro-label pairs — 9px uppercase letter-spaced key (`what's happening`, `why it matters`, `the fix`, `where` — where always last) over 12.5px muted value. `code` in a monospace chip on `--panel-2`.
- Pills (outlined, 999px radius, 10.5px) for severity/status/area: red = blocker, amber = should-fix, muted gray = nit/area, teal = fixed/clean. Filled tint background only for red/teal states.
- A centered `.divider-pill` (2px indigo outline, uppercase) separating findings from the coverage block.
